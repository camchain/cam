...
using UserWallet = Cam.Implementations.Wallets.EntityFramework.UserWallet;

namespace Cam.Implementations.Wallets.NEP6
{
    public class NEP6Wallet : Wallet, IDisposable
    {
        ...
        private Dictionary<UInt160, NEP6Account> accounts;
        ...
        public NEP6Wallet(string path, string name = null)
        {
            ...
            if (File.Exists(path))
            {
                ...
                JArray accountJArray = (JArray)wallet["accounts"];
                foreach (JObject jobect in accountJArray)
                {
                    NEP6Account nEP6Account = NEP6Account.FromJson(jobect, this);
                }
        ...
        public bool ChangePassword(string password_old, string password_new)
        {
            if (!VerifyPassword(password_old)) return false;           
            {
                ...
                Dictionary<UInt160, NEP6Account> newAccounts = new Dictionary<UInt160, NEP6Account>();

                foreach(NEP6Account account in accounts.Values)
                {

                    if (account.HasKey)//只更改本地的账户密码
                    {
                        account.ChangePassword(password_new);
                    }
                    newAccounts.Add(account.ScriptHash, account);                    
                }

                foreach (NEP6Account account in newAccounts.Values.Where(p => p.HasKey))
                {

                    if(!account.VerifyPassword(password_new))//只要有一个账户验证失败则不更新，避免有些账户更新，有些账户未更新，导致资产损失
                    {
                        return false;
                    }
