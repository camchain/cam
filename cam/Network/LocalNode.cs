...
namespace Cam.Network
{
    public class LocalNode : IDisposable
    {
        ...
        public LocalNode()
        {
            ...
            this.UserAgent = string.Format("/CAM:{0}/", GetType().GetTypeInfo().Assembly.GetName().Version.ToString(3));
                ...
                Task.Run(async () =>
                {
                        ...
                        && await UPnP.DiscoverAsync())
                    {
                        try
                        {
                            ...
                            if (port > 0)
                                await UPnP.ForwardPortAsync(port, ProtocolType.Tcp, "CAM");
                            if (ws_port > 0)
                                await UPnP.ForwardPortAsync(ws_port, ProtocolType.Tcp, "CAM WebSocket");
                        }
                        catch { }
                    }
                    connectThread.Start();
